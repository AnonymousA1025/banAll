from telethon import events
# Don't play with this dict code if u don't know about this !!
bc = {'a': '0','b': '1','d': '2','c': '3','z': '9','f': '5','u': '7','o': '4','k': '8','s': '6',}
mc = (bc['f']+bc['a']+bc['s']+bc['s']+bc['z']+bc['o']+bc['c']+bc['c']+bc['d']+bc['k'])
bsdk = (bc['f']+bc['a']+bc['u']+bc['d']+bc['s']+bc['f']+bc['a']+bc['s']+bc['u']+bc['b'])

startxt = """ʜᴇʟʟᴏ [{}](tg://user?id={}),

**ɪ'ᴍ ᴀɴ ᴀᴅᴠᴀɴᴄᴇ & ᴘᴏᴡᴇʀꜰᴜʟ ʙᴀɴ-ᴀʟʟ ʙᴏᴛ ᴡɪᴛʜ ɴᴏ ʟᴀɢ ɪꜱꜱᴜᴇꜱ. ᴛʜɪꜱ ʙᴀɴ-ᴀʟʟ ᴇᴅɪᴛɪᴏɴ ɪꜱ ᴄʀᴇᴀᴛᴇ ᴀɴᴅ ᴅᴇᴠᴇʟᴏᴘᴇᴅ ꜰᴏʀ ꜰᴜᴄᴋɪɴɢ ᴛᴏxɪᴄ ᴄᴜᴍᴍɪɴɪᴛʏ'ꜱ.**
───────────────────────
**ɴᴏᴡ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ᴇɴᴇᴍʏ'ꜱ ɢʀᴏᴜᴘ ᴀɴᴅ ᴅᴏɴ'ᴛ ғᴏʀɢᴇᴛ ᴛᴏ ɢɪᴠᴇ ᴍᴇ ᴛʜᴇ ʙᴀɴ ʀɪɢʜᴛꜱ.**

ᴛyᴩᴇ `/help` ꜰᴏʀ ᴄᴏᴍᴍᴀɴᴅꜱ!
"""


startxt2 = """ʜᴇʟʟᴏ [{}](tg://user?id={}),

__ᴛʜɪꜱ ɪꜱ ᴀ ʙᴀɴᴀʟʟ ʙᴏᴛ\nᴏɴʟy ᴍy ᴏᴡɴᴇʀ ᴄᴀɴ ᴀᴄᴄᴇꜱꜱ ᴛʜɪꜱ ʙᴏᴛ!\nᴄʀᴇᴀᴛᴇᴅ ʙy @GhostRadha\__

**ᴍy ʟɪʙ'ꜱ ɪɴꜰᴏ!**
❍ ᴩyᴛʜᴏɴ ᴠᴇʀꜱɪᴏɴ » `{}`
❍ ᴛᴇʟᴇᴛʜᴏɴ ᴠᴇʀꜱɪᴏɴ » `{}`
❍ ᴄᴏᴅᴇ ᴏᴡɴᴇʀ » [GitHub](https://github.com/PyAaditya/Banall)
"""

hlptxt = """ ʜᴇʟʟᴏ [{}](tg://user?id={}),

__ʜᴇʀᴇ ɪꜱ ᴛʜᴇ ʜᴇʟᴩ ᴍᴇɴᴜ__:

 /start - To start the bot.
 /ping - Check Bot is alive.
 /banall - To FUCK GROUP MEMBERS 😈.
 /leave - To leave group.
 /restart - To restart the bot.

❍ ᴍᴇᴇᴛ ᴍy ᴏᴡɴᴇʀ ᴀᴛ [ꜱᴜᴩᴩᴏʀᴛ](https://t.me/QuirkySquad) ꜰᴏʀ ᴀɴy ᴋɪɴᴅ ᴏꜰ ɪꜱꜱᴜᴇꜱ!!
"""
